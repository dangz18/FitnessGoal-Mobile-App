...
import 'dart:convert';
//Meal Class
import 'package:fitness_goal_android_app/models/meal.dart';
//Global variables
...
class _MealsPageState extends State<MealsPage> {
  ...
  Future getAllMeals() async{
    ...
    if(data == 'Nothing found'){
      ...
      for(int i=0; i<data.length; i++){
        ...
        meals.add(Meal(data[i][0], data[i][1], data[i][2], data[i][3], vegetarian, double.parse(data[i][5]), double.parse(data[i][6]), double.parse(data[i][7]), double.parse(data[i][8]), double.parse(data[i][9]), double.parse(data[i][10]), double.parse(data[i][11]), double.parse(data[i][12]), null, int.parse(data[i][13])));
        meals[i].mealName;
      }
      return true;
    }
  ...
  //Get image for meal
  Future getMealImage(int index) async{
    var url1 = Uri.parse("http://${globals.ipAddress}/fitnessgoaldb/getMealImage.php");
    var response1 = await http.post(url1, body:{
    ...
    if(data == 'Nothing found'){
      ...
      meals[index].mealImage = base64Decode(data);
      return true;
    }
  }
                        ...
                        SizedBox(
                            ...
                            child: new IconButton(
                              ...
                              icon: new Icon(CupertinoIcons.profile_circled,
                                  size: 45.0),
                              onPressed: () {
                                Navigator.push(context, MaterialPageRoute(builder: (context) => ProfilePage(userId: userId))).then((_){
                                  setState(() {
                                    _futureMeals = getAllMeals();
                                  });
                                });
              ...
              Container( //white cont
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      Text('My Meal Plan', style: TextStyle(fontSize: 35, fontFamily: 'Alegreya', color: Colors.black)),
                      FutureBuilder<dynamic>(
                        ...
                        builder: (context, AsyncSnapshot<dynamic> snapshot){
                          if(snapshot.connectionState == ConnectionState.waiting){
                                      ...
                                      itemBuilder: (context, index){
                                        if(0<=index && index<=2){
                                          return Padding(
                                            ...
                                            child: mealDayContainer(todayDate, index),
                                            //child: Text('${index}'),
                                          );
                                        }
                                          ...
                                          return Padding(
                                            padding: EdgeInsets.all(3),
                                            child: mealDayContainer(tomorrowDate, index),
                                          );
                                        }
                                        else if(6<=index && index<=8){
                                          return Padding(
                                            padding: EdgeInsets.all(3),
                                            child: mealDayContainer(afterTomorrowDate, index),
                                          );
                                        }
                                        else{
                                          return Container();
                            ...
                            return Text('No meals found', style: TextStyle(fontFamily: 'calibri', fontSize: 35, fontWeight: FontWeight.bold),);
                          }
                        },
                      ),
                    ],
  ...
  Widget mealDayContainer(String date, int index){
    if(index==0 || index==3 || index==6){
      if(index==0){
        return Column(
            ...
            children: [
              SizedBox(height: 5),
              Text('Today: ${date}', style: TextStyle(fontFamily: 'calibri', fontSize: 30)),
              mealBtn(index),
              SizedBox(height: 20),
            ]
        );
        ...
        return Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Text('Tomorrow: ${date}', style: TextStyle(fontFamily: 'calibri', fontSize: 30)),
              mealBtn(index),
              SizedBox(height: 20),
            ]
        );
        ...
        return Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Text('${date}', style: TextStyle(fontFamily: 'calibri', fontSize: 30)),
              mealBtn(index),
              SizedBox(height: 20),
            ]
        );
  ...
  Widget mealBtn(int index){
    return SizedBox(
      height: 70,
      width: 300,
      child: ElevatedButton(
        child: Text('${meals[index].mealCategory}', style: TextStyle(color: Colors.black, fontSize: 35, fontFamily: 'calibri')),
        onPressed: ()  {
          showDialog(
              context: context,
              builder: (context) {
  ...
  Widget mealContainer(int index){
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        Row(
          ...
          children: [
            Container(
              child: drawImage(index) ?? FutureBuilder<dynamic>(
                future: getMealImage(index),
                builder: (context, AsyncSnapshot<dynamic> snapshot){
                  if(snapshot.connectionState == ConnectionState.waiting){
                    return const Center(child: CircularProgressIndicator(color: Colors.black26));
                  }
                  else if(snapshot.hasData){
                    return drawImage(index) ?? Uint8List.fromList(imageNoFound);
                  }
                  else{
            ...
            Container(
              width: 200,
              child: Text('Name:\n ${meals[index].mealName}', style: TextStyle(fontSize: 25, fontFamily: 'calibri'))
            ),
          ],
        ),
        Text('Ingredients:', style: TextStyle(fontSize: 25, fontFamily: 'calibri')),
        Text('${meals[index].mealIngredients}', style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
        Text('Instructions:', style: TextStyle(fontSize: 25, fontFamily: 'calibri')),
        Text('${meals[index].mealInstructions}', style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
        Text('Additional information', style: TextStyle(fontSize: 25, fontFamily: 'calibri')),
        meals[index].mealVegetarian
            ? Text('Vegetarian meal',style: TextStyle(fontSize: 18, fontFamily: 'calibri'))
            : Text('Not vegetarian meal',style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
        Text('Kcal: ${meals[index].mealKcal}', style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
        Text('Fats: ${meals[index].mealFat} g', style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
        Text('Saturates: ${meals[index].mealSaturates} g', style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
        Text('Carbohydrates: ${meals[index].mealCarbs} g', style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
        Text('Sugar: ${meals[index].mealSugar} g', style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
        Text('Fibre: ${meals[index].mealFibre} g', style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
        Text('Protein: ${meals[index].mealProtein} g', style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
        Text('Salt: ${meals[index].mealSalt} g', style: TextStyle(fontSize: 18, fontFamily: 'calibri')),
      ],
    );
  }

  drawImage(int index){
    if(meals[index].mealImage == null){
      return null;
    }
    else{
      return Container(
        height: 130,
        width: 130,
        child: Image.memory(meals[index].mealImage ?? Uint8List.fromList(imageNoFound)),
      );
    }
  }
