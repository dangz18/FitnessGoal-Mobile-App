...
//Exercise Class
import 'package:fitness_goal_android_app/models/allExercises.dart';
...
class _AddExercisePageState extends State<AddExercisePage> {
  final int workoutDayId;
  _AddExercisePageState({required this.workoutDayId}){
    exercises = [];
    _futureExercises = getAllExercises(_filterController.text);
  }

  late List<AllExercises> exercises;
  late Future _futureExercises;
  ...
  //Get all exercises
  Future getAllExercises(String filter) async{
    exercises.clear();
    var url1 = Uri.parse("http://${globals.ipAddress}/fitnessgoaldb/getAllExercises.php");
    var response1 = await http.post(url1, body:{}).timeout(Duration(seconds: 30));
    ...
    if(data == 'Nothing found'){
      ...
      if(filter.isEmpty){
        //Unfiltered list of exercises
        for(int i=0; i<data.length; i++){
          exercises.add(AllExercises(int.parse(data[i][0]), data[i][1], data[i][2], null));
        }
      }
      else{
        //Filtered list of exercises
        for(int i=0; i<data.length; i++){
          if(data[i][1].toString().toLowerCase().contains(filter.toLowerCase()) || data[i][2].toString().toLowerCase().contains(filter.toLowerCase())){
            exercises.add(AllExercises(int.parse(data[i][0]), data[i][1], data[i][2], null));
          }
          else{}
        }
  ...
  Future getExerciseImage(int index) async{
    var url1 = Uri.parse("http://${globals.ipAddress}/fitnessgoaldb/getExerciseImage.php");
    ...
    if(data == 'Nothing found'){
      ...
      exercises[index].exerciseImage = base64Decode(data);
      return true;
    }
  }

  //Add exercise to workout
  Future addExercise(int exerciseId, String nrOfReps) async{
    var url1 = Uri.parse("http://${globals.ipAddress}/fitnessgoaldb/insertExercise.php");
    var response1 = await http.post(url1, body:{
      "exerciseId" : '${exerciseId}',
      "repetitions" : nrOfReps,
      "workoutDayId" : '${workoutDayId}',
    }).timeout(Duration(seconds: 30));
    var data = await json.decode(response1.body);
    if(data == 'Failure'){
      Fluttertoast.showToast(
        msg: "Failed to add exercise",
      ...
      Fluttertoast.showToast(
        msg: "Exercise added with success",
        toastLength: Toast.LENGTH_SHORT,
        gravity: ToastGravity.BOTTOM,
        fontSize: 20.0,
      );
                  ...
                  child: Stack(
                    children: <Widget>[
                      Row(
                        ...
                        children: [
                          BackButton(),
                          Text('Add Exercise', style: TextStyle(color: Colors.black, fontFamily: 'Alegreya', fontSize: 32)),
                          SizedBox(width: 30),
                        ],
                      ),
                    ],
  ...
  Widget exerciseBtn(int index){
    return SizedBox(
      ...
      child: ElevatedButton(
        child: Center(child: Text('${exercises[index].exerciseName}', style: TextStyle(color: Colors.black, fontSize: 30, fontFamily: 'calibri'))),
        onPressed: ()  {
                      ...
                      return AlertDialog(
                        ...
                        SingleChildScrollView(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text('${exercises[index].exerciseName}', style: TextStyle(fontSize: 25, fontFamily: 'calibri')),
                              drawImage(index) ?? FutureBuilder<dynamic>(
                                future: getExerciseImage(index),
                                builder: (context, AsyncSnapshot<dynamic> snapshot){
                                  if(snapshot.connectionState == ConnectionState.waiting){
                                    return const Center(child: CircularProgressIndicator(color: Colors.black26));
                                  }
                                    ...
                                    return Container(
                                      ...
                                      child: Image.memory(exercises[index].exerciseImage ?? Uint8List.fromList(imageNoFound)),
                                    );
                                  }
                                  else{
                                    return const Text('Nothing found');
                              ...
                              Text('Muscle category: ${exercises[index].exerciseMuscleCategory}', style: TextStyle(fontSize: 25, fontFamily: 'calibri')),
                              SizedBox(height: 10),
                              Text('Enter the number of repetitions:', style: TextStyle(fontSize: 20, fontFamily: 'calibri')),
                              SizedBox(height: 10),
                              nrRepsContainer(),
                              SizedBox(height: 10),
                              Row(
  ...
  drawImage(int index){
    if(exercises[index].exerciseImage == null){
      return null;
    }
    else{
      return Container(
    ...
    return Container(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
            hintText: filterPressed ? _filterController.text : 'Filter',
            border: InputBorder.none,
            contentPadding: EdgeInsets.only(top: 5),
            suffixIcon: IconButton(
              icon: filterPressed ? Icon(Icons.filter_alt_off_outlined, color: Colors.black, size: 30) : Icon(Icons.filter_alt_outlined, color: Colors.black, size: 30),
              onPressed: () async{
                if(!filterPressed){
        ...
        child: ElevatedButton(
            onPressed: () {
              if(_repsController.text.isEmpty){
                Fluttertoast.showToast(
                  msg: "Enter the number of repetitions!",
                  toastLength: Toast.LENGTH_SHORT,
                  gravity: ToastGravity.BOTTOM,
                  fontSize: 20.0,
                );
                ...
                addExercise(exercises[index].exerciseId, _repsController.text);
